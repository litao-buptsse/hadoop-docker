...
export HADOOP_NAMENODE_OPTS="$JMX_OPTS=10201 -Dcom.sun.management.jmxremote -Xmx85G -Xmn6G -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=60 $HADOOP_NAMENODE_OPTS $HADOOP_SERVERS_OPTS -Xloggc:$HADOOP_LOG_DIR/gc-hadoop-namenode.${CURR_DATE}.log -agentlib:jdwp=transport=dt_socket,address=8002,server=y,suspend=n"
...
export HADOOP_JOBTRACKER_OPTS="$JMX_OPTS=10204 -Dcom.sun.management.jmxremote -Xmx25G -XX:NewSize=300M -XX:MaxNewSize=300M $HADOOP_JOBTRACKER_OPTS $HADOOP_SERVERS_OPTS -Xloggc:$HADOOP_LOG_DIR/gc-hadoop-jobtracker.${CURR_DATE}.log -agentlib:jdwp=transport=dt_socket,address=8004,server=y,suspend=n"
